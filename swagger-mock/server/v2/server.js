...
const apiPermission = '/api/permissions';
...
middleware(path.join(__dirname, 'swaggerV2.yaml'), app, function (err, middleware) {
...
// get Permissions collection from DataStore and refresh permissionData tab
  ...
  app.get(apiPermission, function (req, res, next) {
    ...
    if (permissionData.length == 0) {
      res.status(404);
      res.body = returnHttp404("Permission");
    }
    next();
  });
  ...
  app.put(apiPermission + '/:id', function (req, res, next) {
    ...
    if (exist == -1) {
      res.status(404);
      res.send(returnHttp404("Permission"));
    }
    else {
      // req.body.password = cryptPassword(req, res, next);
      next();
